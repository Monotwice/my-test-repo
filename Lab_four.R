...
plot(st_geometry(subset), pch = 16, col = "green", add = TRUE)
...
# plot heavy
q_5 <- classIntervals(meuse_sf$heavy, n = 5, style = "fisher")
pal <- brewer.pal(5, "Reds")
my_cols <- findColours(q_5, pal)
plot(meuse_sf[, "heavy"], col = my_cols, pch = 19)
legend("topleft",
...
plot(meuse_sf, col = "black", bg = my_cols, pch = 22, add = TRUE)
legend("topleft",
       fill = attr(my_cols, "palette"),
       legend = names(attr(my_cols, "table")), bty = "n"
)
...
ggplot() +
  ...
  geom_sf(data = st_sf(geometry = riv), fill = "blue", color = "blue") +
